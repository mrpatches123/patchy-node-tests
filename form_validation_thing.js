...
class Form {
    checkSchema(schema, target, root, stack) {
        Object.entries(schema).forEach(([elementKey, value]) => {
            if (root && elementKey === 'global')
                this.checkSchema(value, target, root, stack);
            else if (elementKey === 'schema')
                this.checkSchema(value, target[elementKey], root, stack);
        });
    }
}
